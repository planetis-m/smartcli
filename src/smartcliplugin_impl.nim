      ...
      if valueAt >= 0:
        ...
        if placeholder.contains('|'):
          ...
          for choice in option.choices:
            option.enumNames.add "cli" & toPascalCase(option.longName) & toPascalCase(choice)
        else:
...
proc emitEnumDecl(b: var Builder; typeName, noneName: string; enumNames: openArray[string]) =
  b.withTree "type":
    b.addIdent(typeName)
    b.addEmpty()
    b.addEmpty()
...
proc emitVarDeclEmpty(b: var Builder; name, typeName: string) =
  b.withTree "var":
    b.addIdent(name)
    b.addEmpty()
    b.addEmpty()
